...
int sumArray = GetSumDiagonal(array);
Console.WriteLine($"Сумма элементов главной диагонали = {GetSumDiagonal(array)}");
...
int GetSumDiagonal(int[,] inArray)
{
    ...
    // }
    for(int i = 0; i < resolutionMin; i++) sum += inArray[i, i];
    return sum;
}